...
1. **λ（市場ブレンド率）の実測**（最優先） — `snapshot_odds.py` のオッズが
   3週間分（約1万レース）たまったら、`0.25` という暫定値が妥当か検証できます。
   同時に「期待値1.05超だけ買う」という**実際の買い方の回収率**を初めて測れます。
   目安は2026年8月中旬。累計は「オッズ収集」ワークフローのログで確認できます。
