...
7. **未測定の候補が2つ残っている**（正直に記録）:
   - **out-of-fold較正** — 較正パラメータ（first + 枠ペア行列、260個超）を
     今は末尾3ヶ月のvalだけで推定している。学習期間をk分割して
     out-of-foldスコアを作れば36ヶ月分で推定でき、ノイズが減るはず。
     週次学習のジョブ内で5回学習する形（+5分程度）で実装可能。期待 0.001〜0.005
   - **モーター機歴の独自集計** — Bファイルのモーター2連率は年次交換直後は
     ほぼ無意味（母数が小さい）。自DBで (場, モーター番号) の expanding 勝率を
     縮小推定すれば補える可能性。期待 0.000〜0.003
