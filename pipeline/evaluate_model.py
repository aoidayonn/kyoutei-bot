"""モデルを指定期間で評価する（採用ゲート用）。

現在の方式: **出荷された状態のまま**採点する
--------------------------------------------
新旧どちらの model.json も温度と段階較正一式を焼き込んで出荷されるので、
実際にデプロイされるアーティファクト同士を同じ窓で採点するのが最も公平。
較正の作り方の改善（val→out-of-fold化など）もゲートに正しく反映される。
主指標は商品そのものである **3連単のNLL**（1〜3着の完全対数尤度）。

歴史的経緯: 旧実装は評価窓で温度・較正を再推定していた。これは
「温度を持たない初期モデル」との比較を公平にするためだった
（実測で LogLoss改善0.036のうち0.025が温度の産物だった事故に由来）。
較正を持たない旧形式モデルが来た場合のみ、後方互換として
評価窓の前半で較正を推定し後半で採点する。

    python evaluate_model.py --model ../pipeline/model_eval.json \
